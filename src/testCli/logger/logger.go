...
func LoadLogOption() {
	logLevelString := os.Getenv("MY_LOG_LEVEL")
	if logLevelString != "" {
		logLevelString = strings.ToLower(logLevelString)
		switch logLevelString {
		case "fatal":
			logOption.logLevel = levelFatal
		case "success":
			logOption.logLevel = levelSuccess
		case "hint":
			logOption.logLevel = levelHint
		case "debug":
			logOption.logLevel = levelDebug
		case "info":
			logOption.logLevel = levelInfo
		case "warn":
			logOption.logLevel = levelWarn
		case "error":
			logOption.logLevel = levelError
		case "critical":
			logOption.logLevel = levelCritical
		default:
			Log.Fatal("unknow log level, not in [fatal, success, hint, debug, info, warn, error, critical]")
		}
	}
}
